...
def main():
    ...
    for img_id, fname in enumerate(img_files, 1):
        ...
        # ---------- 3. 模型推理 ----------
        results = model.predict(img_path, imgsz=IMGSZ, conf=CONF_THRESH, device=0, verbose=False)[0]            #没有进行增强推理TTA的效果会更好，也不需要动iou阈值，直接使用默认的0.7即可
        ...
        if os.path.exists(lbl_path):
            with open(lbl_path, 'r') as f:
                for line in f:
                    ...
                    if cls != 0: continue
                    x_abs = cx * w_img; y_abs = cy * h_img
                    bw_abs = bw * w_img; bh_abs = bh * h_img
                    x1 = x_abs - bw_abs/2; y1 = y_abs - bh_abs/2
        ...
        for box in pred_filt:
            x1, y1, x2, y2, conf = box
            w, h = x2-x1, y2-y1
            pred_anns.append({'id': pred_ann_id, 'image_id': img_id, 'category_id': 1,
                              'bbox': [float(x1), float(y1), float(w), float(h)],
                              'score': float(conf), 'area': float(w*h), 'iscrowd': 0})
            pred_ann_id += 1

        for box in gt_filt_xywh:
            x1, y1, w, h = box
            gt_anns.append({'id': gt_ann_id, 'image_id': img_id, 'category_id': 1,
                            'bbox': [float(x1), float(y1), float(w), float(h)],
    ...
    with open('pred_25m.json', 'r') as f:
        pred_data = json.load(f)
    ...
    precision, recall, f1, accuracy, tp, fp, fn = compute_detailed_metrics(
        'gt_25m.json', 'pred_25m.json', CONF_THRESH
    )
